...
COMPLETED_MEETING_IDS_LOG = 'completed-list.log'
...
def list_recordings_date_specified(email, startdate, enddate):
    recordings = []
    formattedStartDate = datetime.strptime(startdate,'%m/%d/%Y')
    formattedEndDate = datetime.strptime(enddate,'%m/%d/%Y')
    # for start, end in perdelta(date(RECORDING_START_YEAR, RECORDING_START_MONTH, RECORDING_START_DAY), date.today()):
    post_data = get_recordings(email, 300, startdate, enddate)
...
def list_recordings_default(email):
    recordings = []

    # for start, end in perdelta(date(RECORDING_START_YEAR, RECORDING_START_MONTH, RECORDING_START_DAY), date.today()):
    post_data = get_recordings(email, 300, date.today()-timedelta(days=30), date.today())
    response = requests.get(url=API_ENDPOINT_RECORDING_LIST(
        email), headers=AUTHORIZATION_HEADER, params=post_data)
...
def main(email, **kwargs):
    ...
    # for email, user_id, first_name, last_name in users:
        ...
        # time.sleep(0.1)
    if len(kwargs) is not 0 and "startdate" in kwargs and "enddate" in kwargs:
        recordings = list_recordings_date_specified(email, kwargs["stardate"], kwargs["enddate"])
        total_count = len(recordings)
        print("==> Found {} recordings".format(total_count))
        print(recordings)
        return recordings
    else:
        recordings = list_recordings_default(email)
        total_count = len(recordings)
        print("==> Found {} recordings".format(total_count))
        print(recordings)
        return recordings
...
if __name__ == "__main__":
    ...
    main("bettersandboxzoom@gmail.com")
