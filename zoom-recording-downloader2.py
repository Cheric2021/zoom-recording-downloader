...
def main(argv):
    ...
    if "email" in args:
        ...
        if user is not None:
            user_id = user["id"]
            from_date = args["from"] if "from" in args else date(2019, 9, 26)
            now = datetime.now()
            ...
            os.system("rclone copy --progress --transfers 6 " + directory.replace(" ", "\ ") + " remote_google_drive:" + directory.replace(" ", "\ "))
